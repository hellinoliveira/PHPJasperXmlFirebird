...
class PHPJasperXML {
   ...
   public function PHPJasperXML($lang = "en", $pdflib = "TCPDF") {
      $this->lang = $lang;
      $this->db = NewADOConnection('firebird');

      error_reporting(0);
      $this->pdflib = $pdflib;
      $this->fontdir = dirname(__FILE__) . "/tcpdf.6.0/fonts";
   }
...
//### New: =>date("Y.m.d.",....
            $this->pointer[] = array("type" => "MultiCell", "width" => $data->reportElement["width"], "height" => $height, "txt" => date("dd/mm/YYYY"), "border" => $border, "align" => $align, "fill" => $fill, "hidden_type" => "date", "soverflow" => $stretchoverflow, "poverflow" => $printoverflow, "link" => substr($data->hyperlinkReferenceExpression, 1, -1), "valign" => $valign);
//### End of modification
            break;
...
//        $b=$data->subreportParameter;
   ...
   public function transferDBtoArray($host, $user, $password, $db_or_dsn_name, $cndriver = "firebird") {
      $this->m = 0;
      $this->db_host = $host;
      $this->db_user = $user;
      ...
      if (!$this->db->Connect($host, $user, $password, $db_or_dsn_name)) { //connect database
         echo "Fail to connect database!" . $this->db->ErrorMsg();
         exit(0);
      }
...
//$this->arrayVariable['returnValue']['target'] = 'VALOR';
      foreach ($this->arrayVariable as $k => $out) {
         //   echo $out['resetType']. "<br/><br/>";
         switch ($out["calculation"]) {
            case "Sum":
               ...
               if ($out['resetType'] == '') {
                  ...
                  if (isset($this->arrayVariable[$k]['class']) && $this->arrayVariable[$k]['class'] == "java.sql.Time") {
                     ...
                     //print_r($out)."<br>";
                     //print_r($this->arraysqltable[$rowno])."<br>";
                     $value+=$this->arraysqltable[$rowno]["$out[target]"];
                      //echo $value."<br>";
                  }
               }
...
//echo "$pchartfolder/class/pData.class.php";die;
      ...
      $result = $this->db->Execute($sql) or die("Erro Codificacao. Função showlinecart. PHPJasperXMLFB"); //query from db
      $chartdata = array();
      $i = 0;
//echo $sql."<br/><br/>";
      $seriesname = array();
      ...
      while ($row = $result->FetchRow()) {
         print "teste_linha 1448 jasperxmlFB.inc";
         $j = 0;
...
//echo $sql."<br/><br/>";
      ...
      $this->db->SetFetchMode(ADODB_FETCH_ASSOC);
      while ($row = $result->FetchRow()) {
         print "teste_linha 1743 jasperxmlFB.inc";
         $j = 0;
         foreach ($row as $key => $value) {
            //$chartdata[$j][$i]=$value;
            if ($value == '')
...
//echo $sql."<br/><br/>";
      ...
      $this->db->SetFetchMode(ADODB_FETCH_ASSOC);
      while ($row = $result->FetchRow()) {
         print "teste_linha 2045 jasperxmlFB.inc";
         $j = 0;
         foreach ($row as $key => $value) {
            //$chartdata[$j][$i]=$value;
            if ($value == '')
...
//#,##0.00
   public function formatText($txt, $pattern) {
      ...
      elseif ($pattern == "###0.0000")
         return number_format($txt, 4, ".", "");
      elseif ($pattern == "dd/MM/yyyy" && $txt != "") { //verifica se a data tem formato válido. Foi Necessário alterar a sql pra cast (coluna as date)
         $time = strtotime($txt);
         $date = ($time === false) ? '00/00/0000' : date('d/m/Y', $time);
         return $date;
      ...
      elseif ($pattern == "dd/MM/yyyy HH.mm.ss" && $txt != "") //Formato de hora usado no Header do relatório.
         return date("d/m/Y H:i:s", strtotime($txt));
      elseif ($pattern == "cpfcnpj" && $txt != "") {//validação de CPF e CNPJ
         $output = preg_replace("[' '-./ t]", '', $txt);
         $size = (strlen($output) - 2);
         if ($size != 9 && $size != 12)//37933540910
            return false;
         $mask = ($size == 9) ? '###.###.###-##' : '##.###.###/####-##';
         $index = -1;
         for ($i = 0; $i < strlen($mask); $i++):
      ...
      elseif ($pattern == "cliente" && $txt != "") {
         $sizeCobrador = strlen($txt);
         //print $sizeCobrador."<br>";
         if ($sizeCobrador > 30):
            if (!strpos($txt, ' '))://se nao encontrar espacos, imprime
               return substr($txt, 0, 30); //Se nao houver espacos, pega as 30 primeiras letras do nome
            else :
               return substr(substr($txt, 0, 30), 0, strrpos($txt, ' ')); //mostra ate o ultimo espaco em 30 letras
            endif;
         else:
            return $txt;
         endif;
...
//                $arrdata[$num]=str_replace('"',"",$out);
   ...
   public function includeSubReport($d, $arrdata, $current_y) {
      //print_r($arrdata);
      //print_r($this->arrayVariable);
      include_once ("PHPJasperXMLSubReportFB.inc.php");
      ...
      // echo $this->db_host. "-" .$this->db_user. "-" .$this->db_pass. "-" .$this->db_or_dsn_name;
      ...
      //print_r($PHPJasperXMLSubReport);
      $PHPJasperXMLSubReport->outpage($this->arrayVariable);    //page output method I:standard output  D:Download file
      ...
      //$PHPJasperXMLSubReport->arrayVariable['returnValue']['target'] = 'VALOR';
      //print_r($PHPJasperXMLSubReport->arrayVariable);
